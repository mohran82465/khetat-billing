            ...
            <div className="bg-white rounded-xl p-4 border border-[#e3e8f9] shadow-xs flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-[#161c27]">
                  {isArabic ? 'كتالوج باقات نظام الضيافة السعودي' : 'Hospitality OS Subscription Plans'}
                </h2>
                <p className="text-xs text-[#70787d]">
                  {isArabic
                    ? 'تسعير مخصص حسب عدد المفاتيح والغرف أو باقات موحدة مع ترخيص ZATCA السحابي'
                    : 'Turnkey SaaS plans for independent hotels, boutique chalets, and apartment chains across KSA.'}
                </p>
              </div>
              <button
                onClick={() => setIsAddModalOpen(true)}
                className="rounded-lg bg-[#004a60] px-3.5 py-2 text-xs font-semibold text-white shadow-xs hover:bg-[#074e64]"
              >
                + {isArabic ? 'إنشاء باقة جديدة' : 'Create Custom Tier'}
              </button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {HOSPITALITY_PLANS.map((plan) => (
                <div
                  key={plan.id}
                ...
                >
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#004a60] text-white px-3 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase shadow-sm">
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-[#004a60] bg-[#e8eeff] px-2 py-0.5 rounded-md">
                        {plan.badge}
                      </span>
                      <span className="text-[11px] font-semibold text-[#70787d]">{plan.category}</span>
                    </div>

                    <h3 className="text-base font-bold text-[#161c27]">{plan.name}</h3>
                    <p className="text-xs text-[#70787d] mt-1 leading-relaxed">
                      {isArabic ? plan.descriptionAr : plan.description}
                    </p>

                    <div className="my-4 pt-3 border-t border-[#e3e8f9]">
                      <div className="flex items-baseline gap-1">
                        <span className="text-xs font-semibold text-[#70787d]">SAR</span>
                        <span className="text-2xl font-black text-[#161c27]">{plan.basePrice}</span>
                        <span className="text-[10px] text-[#70787d]">/{plan.billingFrequency}</span>
                      </div>
                      <div className="text-[10px] font-medium text-emerald-700 mt-1 flex items-center gap-1">
                        <ShieldCheck className="h-3 w-3" />
                        {plan.keyLimit}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="text-[11px] font-bold text-[#161c27] uppercase tracking-wider">
                        {isArabic ? 'المزايا المضمنة:' : 'Included Capabilities:'}
                      </div>
                      <ul className="space-y-1.5 text-xs text-[#40484d]">
                  ...
                  <div className="mt-6 pt-4 border-t border-[#e3e8f9]">
                    <button
                      onClick={() => {
                      ...
                      className={`w-full rounded-xl py-2 text-xs font-bold transition-all shadow-xs ${
                        plan.popular
                          ? 'bg-[#004a60] text-white hover:bg-[#074e64]'
                          : 'bg-[#f1f3ff] text-[#004a60] hover:bg-[#e8eeff]'
                      }`}
                    >
                      {isArabic ? 'اختيار هذه الباقة' : 'Subscribe Property'}
                    </button>
                  </div>
                </div>
              ))}
